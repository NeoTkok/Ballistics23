...
TEST(CONVERTER, GCRS_ITRS) {
    ...
    for (const auto &set : earthRotationResult) {
        const Time<TimeScale::TT_SCALE> tt = Time<TimeScale::TT_SCALE>::buildFromJD(set[0]);

        const Ballistics23::Matrix3d conversionMatrix = frameConverter.conversionMatrixGCRStoITRS(tt);
