...
TEST(EPHEMERIS, POSVEL) {
    ...
    const Ballistics23::Containers::string mjdDutPath = "../../../data/frame/earth_rotation.csv";
    const Ballistics23::Utility::MJD_DUT mjd_dut = Ballistics23::Utility::CSVgetMJD_DUT(mjdDutPath, begin, end);

    const Ballistics23::TimeModule::DutContainer dutContainer(mjd_dut.mjdVector, mjd_dut.dutVector);
    ...
    for (const auto &set : ephemeris_result) {
        const Ballistics23::TimeModule::Time<Ballistics23::TimeModule::TimeScale::TT_SCALE> tt(set[1], set[2]);
        const Ballistics23::TimeModule::Time<Ballistics23::TimeModule::TimeScale::TDB_SCALE> tdb =
            timeConverter.convert<Ballistics23::TimeModule::TimeScale::TDB_SCALE>(tt);
        ...
        const Ballistics23::Containers::vector<double> reference = {set[3], set[4], set[5], set[6], set[7], set[8]};
...
TEST(EPHEMERIS, POS) {
    ...
    for (const auto &set : ephemeris_result) {
        ...
        const Ballistics23::Containers::vector<double> reference = {set[3], set[4], set[5]};
