...
public:
  FrameConverter(const PolarMotionContainer &polarMotionContainer,
      ...
      : polarMotionContainer_(polarMotionContainer),
   ...
   */
  [[nodiscard]] Matrix3<scalar> conversionMatrixGCRStoITRS(
      const TimeModule::Time<TimeModule::TimeScale::TT_SCALE> &tt) const;

  /**
   * Кватернион перехода от GCRS в ITRS
   */
  [[nodiscard]] Quaternion<scalar> quaternionGCRStoITRS(
      const TimeModule::Time<TimeModule::TimeScale::TT_SCALE> &tt) const;
};

template <typename PolarMotionContainer, typename DutContainer>
Quaternion<scalar>
FrameConverter<PolarMotionContainer, DutContainer>::quaternionGCRStoITRS(
    const TimeModule::Time<TimeModule::TimeScale::TT_SCALE> &tt) const {
...
template <typename PolarMotionContainer, typename DutContainer>
Matrix3<scalar>
FrameConverter<PolarMotionContainer, DutContainer>::conversionMatrixGCRStoITRS(
    const TimeModule::Time<TimeModule::TimeScale::TT_SCALE> &tt) const {

  double cipX,
      cipY; // X, Y coordinates of celestial intermediate pole from series based
  ...
  const double cioLocator =
      iauS06(ttInt, ttFrac, cipX, cipY); // в радианах, aka "s"
  ...
  const TimeModule::Time<TimeModule::TimeScale::UT1_SCALE> ut1 =
      timeConverter_.convertTT_UT1(tt);
  ...
  const TimeModule::Time<TimeModule::TimeScale::UTC_SCALE> utc =
      timeConverter_.convertUT1_UTC(ut1);
  ...
  resultMatrix << static_cast<scalar>(rc2it[0][0]),
      static_cast<scalar>(rc2it[0][1]), static_cast<scalar>(rc2it[0][2]),
      static_cast<scalar>(rc2it[1][0]), static_cast<scalar>(rc2it[1][1]),
      static_cast<scalar>(rc2it[1][2]), static_cast<scalar>(rc2it[2][0]),
      static_cast<scalar>(rc2it[2][1]), static_cast<scalar>(rc2it[2][2]);
