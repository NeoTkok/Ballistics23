...
private:
  ...
  [[nodiscard]] InputParams calcInputParam(
    ...
    const TimeModule::Time<TimeModule::TimeScale::TDB_SCALE> timeTDB =
    ...
    const Vector3d jupiterPositionGCRS = stdToVector3d(
        ephemerisCalculator_.calculateBodyR(5, centerBody, timeTDB));

    const Containers::vector<double> sunRVgcrs =
        ephemerisCalculator_.calculateBodyRV(11, centerBody, timeTDB);
    ...
    const Vector3d moonPositionITRS =
        quaternionGCRStoITRS._transformVector(moonPositionGCRS);
    const Vector3d jupiterPositionITRS =
        quaternionGCRStoITRS._transformVector(jupiterPositionGCRS);
    const Vector3d sunPositionITRS =
        quaternionGCRStoITRS._transformVector(sunPositionGCRS);
    const Vector3d sunVelocityITRS =
        quaternionGCRStoITRS._transformVector(sunVelocityGCRS);
  ...
  auxFunction(const std::tuple<OtherForces...> &otherForces,
              const TimeTT timeTT, const Vector3d &positionGCRS,
              const Vector3d &velocityGCRS, double mass,
    ...
    return (std::get<Is>(otherForces)
                .calcAcceleration(timeTT, positionGCRS, velocityGCRS, mass,
...
public:
  ...
  calcAcceleration(const EarthGravity &earthGravity,
                   ...
                   const TimeTT timeTT, const Vector3d &positionITRS,
    ...
    const Vector3d otherForcesAcceleration = auxFunction(
        otherForces, timeTT, positionITRS, velocityITRS, mass, allSatParams,
        inputParams,
