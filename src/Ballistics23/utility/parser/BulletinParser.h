...
std::string getRootPath(const std::string &filePath, int stepsBack) {
  ...
  for (int i = 0; i < stepsBack; ++i) {
    ...
    if (lastSeparator == std::string::npos) {
      lastSeparator =
          rootPath.rfind('\\'); //  попробуем обратный слеш для Windows
    }
...
[[nodiscard]] std::vector<scalar> getColumn(const std::string &path,
                                            const indexType column,
                                            const indexType &compareColumn,
                                            const scalar Begin,
...
ReplaceAll(Containers::string str, const Containers::string &from,
 ...
 */
[[nodiscard]] MJD_DUT bulletinGetMJD_DUT(const Containers::string &path,
                                         const scalar mjdBegin,
                                         const scalar mjdEnd,
  ...
  if (inputFile.is_open()) {
    ...
    while (!inputFile.eof()) {
      ...
      if (counter >= 15) {
        ...
        const Ballistics23::Containers::vector<Containers::string>
            stringVector = split(resultLine, delimiter);
 ...
 * @return 2 массива: mjd и соответствующие dut
 */
[[nodiscard]] MJD_DUT CSVgetMJD_DUT(const Containers::string &path,
                                    const scalar mjdBegin,
  ...
  if (inputFile.is_open()) {
    ...
    while (!inputFile.eof()) {
      ...
      if (counter >= 2) {
        ...
        const Containers::string delimiter = ",";
        const Ballistics23::Containers::vector<Containers::string>
            stringVector = split(line, delimiter);
