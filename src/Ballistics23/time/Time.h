...
inline constexpr std::array<std::string_view, 7> SCALES = {UT1, UTC, TAI, TT,
                                                           TCG, TCB, TDB};

template <TimeScale> class Time {
  scalar jdDayInt_;
  scalar jdDayFrac_;

public:
  static constexpr scalar JD_MINUS_MJD =
      static_cast<scalar>(2400000) + static_cast<scalar>(0.5);
...
constexpr Time<Scale> buildFromCalendar(int year, int month, int day, int hour,
                                        int minute, scalar seconds) {

  // потому что SOFA принимает double
  double jdDay;
  double jdDayPart;

  const int status = static_cast<int>(iauDtf2d(
      static_cast<std::string>(SCALES[static_cast<indexType>(Scale)]).c_str(),
      year, month, day, hour, minute, seconds, &jdDay, &jdDayPart));

  Exceptions::sofaErrorHandler(status);

  return Time<Scale>(static_cast<scalar>(jdDay),
                     static_cast<scalar>(jdDayPart));
}
...
template <TimeScale Scale> constexpr void Time<Scale>::align() {
  const int jdIntFloor = jdDayInt_ >= static_cast<scalar>(0)
                             ? static_cast<int>(jdDayInt_)
                             : static_cast<int>(jdDayInt_) - 1;
  const int jdFracFloor = jdDayFrac_ >= static_cast<scalar>(0)
                              ? static_cast<int>(jdDayFrac_)
                              : static_cast<int>(jdDayFrac_) - 1;
...
template <TimeScale Scale>
constexpr Time<Scale>::Time() noexcept
    : jdDayInt_(static_cast<scalar>(0)), jdDayFrac_(static_cast<scalar>(0)) {}

template <TimeScale Scale>
constexpr Time<Scale>::Time(const scalar jdInt, const scalar jdFrac) noexcept
    : jdDayInt_(jdInt), jdDayFrac_(jdFrac) {
  align();
}
...
template <TimeScale Scale> constexpr scalar Time<Scale>::jd() const noexcept {
  return jdDayInt_ + jdDayFrac_;
}

template <TimeScale Scale> constexpr scalar Time<Scale>::mjd() const noexcept {
...
Time<Scale>::operator-(const scalar seconds) const noexcept {
